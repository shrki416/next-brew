...
const getUrlForBreweries = (latLong) => {
  return `https://api.openbrewerydb.org/breweries?by_dist=${latLong}&per_page=15`
}

export const fetchBreweries = async (latLong = '40.7831,-73.9712', per_page = 15) => {
  try {
    const images = await getListOfBreweryImages()
    const response = await fetch(getUrlForBreweries(latLong))
